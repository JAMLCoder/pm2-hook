        ...
        pm2.launchBus(function (err, bus) {
            bus.on('process:event', function (proc) {
                if (proc && proc.event == 'online' && !_.has(webhooks, proc.process.name)) {
                    ...
                    if (port <= 1024) {
                        console.log( (new Date() ).toString() + ':' +  'Error! Port must be greater than 1024, you are trying to use', port);
                        return;
                    }
                    ...
                    try {
                        ...
                        console.log( (new Date() ).toString() + ':' +  'Error occurs while creating server', error);
                    }
                ...
                if (proc && proc.event == 'exit' && _.has(webhooks, proc.process.name)) {
                    try {
                        ...
                        console.log( (new Date() ).toString() + ':' +  'Error occurs while removing server', error);
                    }
...
function processList(processes) {
    ...
    processes.forEach(function (proc) {
        ...
        if (!_.result(proc,"pm2_env", false) || !_.result(proc, "pm2_env.env_webhook", false) || !_.result(proc,"pm2_env.env_webhook.port", false)) {
            console.log( (new Date() ).toString() + ':' +  'Environment problem for process', proc.name);
            return;
        }
...
function processRequest(port, url, body, headers) {
    ...
    for (const name of Object.keys(webhooks)) {
        ...
        checkOrigin
            ...
            .catch(function () {
                console.log( (new Date() ).toString() + ':' +  'Something went wrong while chocking origin (Application:', name, ')');
            });
    }
}
...
function pullAndReload(name) {
    ...
    async.series([
        ...
        if (err) {
            console.log( (new Date() ).toString() + ':' +  'An error has occuring while processing app', name);
